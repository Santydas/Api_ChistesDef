...
import com.example.chistesyfrases.Repositories.ChistesYFrasesRepository;
...
public class ChistesYFrasesService {
    ...
    public ResponseEntity<?> deleteChiste(UUID id) {
        ...
        if (movie.isEmpty()) {
            return new ResponseEntity<>(Collections.singletonMap("Status", String.format("El chiste con ID %s no existe", id)), HttpStatus.NOT_FOUND);
        }
